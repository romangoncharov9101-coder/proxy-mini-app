...
async def extend_proxies_service(
    ...
    if current_user.role == UserRole.admin:
        ...
        tx = Transaction(
            ...
            order_id=order_id.get('data').get('order_id'),
            api_key_id=user_api_key.id,
            type=TransactionType.extend,
            amount=-total_cost,
            description=f'Продление {len(proxies)} прокси на {days} дн. ({proxy_ids_str})',
        )
        db.add(tx)
        await db.commit()

        logger.info(f'[EXTEND] OK user_is={current_user.id} proxies={len(proxies)} days={days} cost={total_cost}')
        return {
            'status': 'success',
